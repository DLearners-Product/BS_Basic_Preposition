...
public class T7ClickManager : MonoBehaviour
{
  ...
  //  public GameObject activityCompleted;
    ...
    public void OnOptionSelected(Button selectedOption)
    {
        ...
        Text textComponent = selectedOption.gameObject.transform.GetChild(1).GetComponent<Text>(); // Get the Text component from the second child

        if (textComponent == null)
        {
            Debug.LogWarning("Text component not found on child (1) of the selected option.");
            return;
        }
        ...
        else if (optionTag == "Wrong")
        {
            ...
            // Revert the color back to its original after 1 second
            StartCoroutine(RevertTextColorAfterDelay(textComponent, originalColor, 1f));
    ...
    private IEnumerator RevertTextColorAfterDelay(Text textComponent, Color originalColor, float delay)
    {
